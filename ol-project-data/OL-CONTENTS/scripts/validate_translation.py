...
# 상대 경로 로그 디렉토리 위치를 동적으로 결정하기 위한 헬퍼
...
def check_spelling_and_style(ko_paragraphs, is_draft2=False):
    errors = []
    warnings = []
    
    # 1. Spelling/Typo patterns
    typo_dict = {
        r'받아들집니다': '받아들입니다',
        r'덧붙입인다': '덧붙입니다',
        r'했읍니다': '했습니다',
    ...
    # Broken Hangul Jamo regex (e.g. ㄱ, ㄴ, ㅏ, ㅓ)
    jamo_pattern = re.compile(r'[\u3131-\u318e]')
    
    # Passive markers (for draft-2)
    passive_patterns = [
        (re.compile(r'에\s+의해(?:서)?'), "에 의해(서)"),
        (re.compile(r'되어지다'), "되어지다"),
        (re.compile(r'해지다'), "해지다"),
        (re.compile(r'당하다'), "당하다"),
    ]
    
    # Have-verb translations
    have_patterns = re.compile(r'가집니다|가졌다|가지고\s+있다')
    
    # Raw Pali words without Korean explanation
    pali_keywords = ['pāramī', 'cāga', 'cariya', 'adhimuttimaraṇa', 'pacceka', 'sāvaka', 'abhinīhāra', 'ñāṇa', 'bhāvanā', 'asaṅkhyeyya', 'khantī', 'sīla', 'dāna']
    
    for idx, para in enumerate(ko_paragraphs, 1):
        clean_para = re.sub(r'\[\^\d+\]', '', para) # remove footnotes
        
        # Check broken Hangul Jamo
        if jamo_pattern.search(clean_para):
            errors.append(f"[Spelling Error] Block {idx}: Broken/unjoined Hangul Jamo detected in translation block.")
            
        # Check typo dict
        for pattern, replacement in typo_dict.items():
            if re.search(pattern, clean_para):
                warnings.append(f"[Spelling Warning] Block {idx}: Potential typo matching '{pattern}' detected. Did you mean '{replacement}'?")
                
        # Style checks (Draft-2 only)
        if is_draft2:
            for regex, term in passive_patterns:
                if regex.search(clean_para):
                    warnings.append(f"[Style Warning] Block {idx}: Passive voice expression '{term}' detected. Consider converting to active voice for better readability.")
                    
            if have_patterns.search(clean_para):
                warnings.append(f"[Style Warning] Block {idx}: Literal 'have' translation (가지다/가지고 있다) detected. Consider using a more natural Korean verb.")
                
            eng_words = re.findall(r'[a-zA-Zāīūṅñṭḍṇḷṃā́íúū́\-\(\)]+', clean_para)
            for word in eng_words:
                cleaned_word = word.strip('()-')
                norm_word = cleaned_word.lower()
                if norm_word in pali_keywords or any(k in norm_word for k in pali_keywords if len(k) > 4):
                    # Check if the word is inside parenthesis, brackets, or asterisks
                    inside_formatting = False
                    parens = re.findall(r'\((.*?)\)', para)
                    for p in parens:
                        if cleaned_word in p:
                            inside_formatting = True
                            break
                    brackets = re.findall(r'\[(.*?)\]', para)
                    for b in brackets:
                        if cleaned_word in b:
                            inside_formatting = True
                            break
                    asterisks = re.findall(r'\*(.*?)\*', para)
                    for a in asterisks:
                        if cleaned_word in a:
                            inside_formatting = True
                            break
                            
                    if not inside_formatting:
                        warnings.append(f"[Style Warning] Block {idx}: Raw Pali term '{cleaned_word}' found without parentheses or formatting. In 2nd Draft, minimize Pali jargon or wrap it in explanations/parentheses.")
                        
        # Sentence ending consistency check (all drafts) - strip quotes first to prevent false alerts on quotes/thoughts
        clean_para_no_quotes = re.sub(r'["“‘\'](.*?)["”’\']', '', clean_para)
        sentences = [s.strip() for s in re.split(r'[\.\?\!]', clean_para_no_quotes) if s.strip()]
        polite_count = 0
        literary_count = 0
        for sentence in sentences:
            ...
            if not words:
                continue
            last_word = words[-1]
            if last_word.endswith(('니다', '습니다', '요', '오', '지요', '까요', '네요')):
                polite_count += 1
            elif last_word.endswith(('다', '라', 'ㄴ다', '았다', '었다', '였다', '한다', '하리라')):
                if not last_word.endswith(('니다', '습니다')):
                    literary_count += 1
                    
        if polite_count > 0 and literary_count > 0:
            if polite_count >= 1 and literary_count >= 1:
                warnings.append(f"[Style Warning] Block {idx}: Inconsistent sentence endings detected. Block contains {polite_count} polite ending(s) and {literary_count} literary ending(s). Please align the tone.")
                
    return errors, warnings

def validate_file(file_path, is_draft2=False, lang=None, project=None):
    path = Path(file_path)
    if not path.exists():
    ...
    spelling_errors, spelling_warnings = check_spelling_and_style(ko_paragraphs, is_draft2=is_draft2)
    
    all_passed = yaml_ok and block_align_ok and markers_ok and footnotes_ok and (len(spelling_errors) == 0)
    ...
    print(f"[*] Spelling & Style Check: {'Pass' if not spelling_errors else 'Fail'}")
    
    if spelling_errors or spelling_warnings:
        print("\n--- Spelling & Style Analysis ---")
        for err in spelling_errors:
            print(f"Error: {err}")
        for warn in spelling_warnings:
    ...
    log_dir = paths["log_dir"] / sub_folder
