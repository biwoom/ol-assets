...
이 스킬은 `scripts/` 하위에 위치하며 다음과 같이 구성됩니다.

1. **`SKILL.md`** (본 문서): 스킬 정의 및 통합 가이드
2. **`run_pipeline.py`**: 전처리, 최종 검증 및 자가 업그레이드 단계를 일괄 제어하는 원스톱 파이프라인 관리자
3. **`format_footnotes.py`**: 위키 주석 변환 및 한국어 각주 통합 래핑 전처리 도구
4. **`lint_glossary.py`**: `gcb-revised-term.md` 용어집 준수 검사 린터
5. **`validate_translation.py`**: 1차 및 2차 정렬/각주 검증 및 자가 검증 로그 생성기
...
```
*참고: 입력 파일에 실제 본문이 없는 빈 문서로 판명되면, 실행 즉시 바이패스가 감지되어 `log/en-kr/BuddhaStory/draft-1/` 폴더에 즉시 예외 처리 로그를 자동 생성하고 처리를 완료합니다.*

#### 2단계: 번역 수행
1차 번역 규칙에 따라 영어 문단 바로 아래에 `[KO]` 번역 문단을 1:1로 추가합니다.

#### 3단계: 통합 검증 및 로그 생성 (--validate)
번역 완료 후 용어집 검사(린팅)와 구조 정합성 검수를 일괄 처리하고, 이상 없을 시 1차 자가 검증 로그 파일(`log/en-kr/BuddhaStory/draft-1/[대상파일명]-log.md`)을 자동 생성합니다.
```bash
...
1차 번역 파일을 `draft-2/`로 복사하고 용어집 파일(`term/en-kr/BuddhaStory/[대상파일명]-term.md`)에 `2차 :` 서식을 추가해 템플릿화합니다.
```bash
...
2차 원고의 포맷을 검수하고, 2차 전용 자가 검증 로그 파일(`log/en-kr/BuddhaStory/draft-2/[대상파일명]-draft2-log.md`)을 자동 생성합니다. 또한 검증 성공 시 인간 감수자의 최종 검토를 위해 원고를 `translation/en-kr/BuddhaStory/draft-3/` 폴더로 자동 복사합니다.
```bash
...
2차 자가 검증 로그 내의 피드백 사항을 분석하여 룰 문서(`GEMINI.md`, `UBTF-GLOBAL-MANUAL.md` 등)를 스스로 개정/갱신합니다.
```bash
