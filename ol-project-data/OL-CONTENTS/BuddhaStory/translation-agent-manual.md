...
| **3차 번역본 경로** | `BuddhaStory/edit/gcb-kr/draft-3/` | 인간 감수자가 최종 출판을 위해 추가 윤문 및 편집을 수행할 최종 원고 경로 |
| **누적 용어집 경로** | `BuddhaStory/term/gcb-revised-term.md` | 승인된 용어가 추가되는 불교 용어 및 고유명사 누적 관리 파일 |
| **작업 로그 경로** | `BuddhaStory/log/draft-1/` 및 `draft-2/` | AI 모델이 원고 번역 및 자체검증 후 기록을 남기는 분할 경로 (한국어로 작성) |
...
### 5.3 본문 내용이 없는 빈 문서 예외처리 규칙

- 입력 파일 분석 단계에서 본문 텍스트가 존재하지 않고, 단순 챕터명(예: `# Chapter 2 - ...`), `## 본문` 헤더, 그리고 상호 참조 주석(예: `[For the Anudīpanī on this chapter...]`) 한 줄만 존재하는 경우에는 **번역 작업 대상에서 예외 처리(Bypass)**합니다.
- 이러한 빈 문서의 경우 실제 본문 번역이나 헬퍼 스크립트 실행 등 모든 실질 가공 단계를 생략합니다.
- 단, 번역 누락 의심을 방지하기 위해 **해당 사실을 작업 로그(`[파일명]-log.md`)에 예외 처리 사유(예: "본문 내용이 없는 빈 문서이므로 번역 예외 처리함")와 함께 기록에 명시**해야 합니다.
...
1차 원고 제작은 자동화 스킬 도구(`buddha-translation-helper`)의 통합 파이프라인 관리 스크립트(`run_pipeline.py`)를 활용하여 각 단계에서 기계적 검증 및 전처리를 수행합니다.

### 16.1 자동화 스크립트 실행 절차

1. **[번역 전] 각주 및 마크업 전처리**:
   대상 원고 파일의 위키 주석을 마크다운 포맷으로 변환하고 하단 각주 영역을 정형화합니다.
   ```bash
   python3 BuddhaStory/skills/buddha-translation-helper/scripts/run_pipeline.py --preprocess [번역대상파일.md]
   ```
   *참고: 만약 대상 원고 파일이 본문 텍스트가 존재하지 않는 빈 문서인 경우, 본 단계 또는 다음 단계의 실행 과정에서 자동으로 바이패스가 감지되어 `log/` 폴더에 즉시 예외 처리 로그를 자동 생성하고 처리를 우회합니다.*

2. **[번역 전] 신규 용어 추출 및 마크다운 생성**:
   기존 누적 용어집(`gcb-revised-term.md`)을 읽어 숙지한 후, 번역할 문서에서 아직 용어집에 등재되지 않은 새로운 고유명사나 교학 용어를 추출합니다. 추출한 신규 용어는 `BuddhaStory/term/[문서명]-term.md` 문서로 생성하여 저장합니다. **이후 사용자의 사전 승인을 대기하지 않고, 추출된 용어에 의거하여 즉시 나머지 번역 워크플로우를 계속 집행합니다.**

3. **[번역 단계] 1:1 번역 및 KO 마크업 수행**:
   영어 문단 아래에 `[KO]...[/KO]`를 1:1로 삽입하여 번역을 작성합니다. (1차 번역 기본 원칙 준수)

4. **[번역 후] 용어집 일관성 검사, 자체 검증 및 로그 자동 생성**:
   용어집 대조 린터(`lint_glossary.py` 연동)와 구조 정합성 검증(`validate_translation.py` 연동)을 일괄 수행하고, 전원 통과 시 `BuddhaStory/log/draft-1/` 내에 자가 검증 로그(`[파일명]-log.md`)를 자동 생성합니다.
   ```bash
   python3 BuddhaStory/skills/buddha-translation-helper/scripts/run_pipeline.py --validate [번역대상파일.md]
   ```
...
graph TD
    ...
    B --> C[Plan 수립: 대화형은 사용자 제시 / 로컬 자동은 로그 기록]
    C --> D{빈 문서 감지?}
    D -- Yes --> E[run_pipeline.py 실행하여 바이패스 로그 자동 생성 및 우회 종료]
    D -- No --> F[1단계: run_pipeline.py --preprocess 실행]
    F --> G[누적 용어집 gcb-revised-term.md 정독 및 숙지]
    G --> H[2단계: 번역문서에서 신규용어 추출하여 BuddhaStory/term에 마크다운 문서 생성]
    H --> I[3단계: 승인 대기 없이 누적용어집과 신규용어에 근거하여 1차 번역 진행]
    I --> J[YAML Frontmatter 및 tagAliases 작성]
    J --> K[4단계: run_pipeline.py --validate 실행 및 자동 자가검증]
    K --> L[BuddhaStory/log/draft-1에 작업 로그 자동 파일 저장 완료]
    L --> M[인간 감수자 최종 검토 및 신규 추출용어 사후 승인 요청]
    M --> N[신규용어들을 인간 감수자의 요청에 따라 누적용어집에 추가 후 작업 완료]
```
...
2차 원고 제작은 1차 번역본을 바탕으로 독자의 가독성을 극대화하는 윤문 단계이며, 스킬 도구(`run_pipeline.py`)의 2차 모드를 적용하여 진행합니다.

### 17.1 자동화 스크립트 실행 및 피드백 절차

1. **[번역 전] 2차 작업 환경 세팅 및 용어 고도화**:
   대상 원고의 1차 번역본을 `draft-2/` 폴더로 복사하고, `term/[파일명]-term.md` 용어집 템플릿에 `* 2차 :` 라인을 자동 추가합니다.
   ```bash
   python3 BuddhaStory/skills/buddha-translation-helper/scripts/run_pipeline.py --init-draft2 [1차원고파일.md]
   ```
   이후 생성된 용어집에서 2차 윤문에 적합한 가독성 높은 윤문용 권장 표기를 기록하고 이를 기준으로 윤문 작업을 착수합니다.

2. **[번역 단계] 독자 지향적 윤문 수행**:
   `draft-2/`에 복사된 파일의 `[KO]...[/KO]` 영역을 수정하며 유려한 한국어로 리라이팅을 진행합니다. (2차 윤문 규칙 준수)

3. **[번역 후] 2차 번역 검증, 2차 로그 자동 생성 및 draft-3 복사**:
   2차 번역 원고의 포맷을 검사하고, 2차 전용 자가 검증 로그(`log/draft-2/[파일명]-draft2-log.md`)를 생성합니다. 검증이 통과되면 인간 감수자가 전적으로 진행할 3차 리뷰를 위해 원고가 `edit/gcb-kr/draft-3/` 경로로 자동 복사됩니다.
   ```bash
   python3 BuddhaStory/skills/buddha-translation-helper/scripts/run_pipeline.py --validate-draft2 [2차원고파일.md]
   ```

4. **[최종 진화] 워크플로우 및 규칙 자가 업그레이드 (Self-Evolution)**:
   생성된 2차 로그의 `## 8` 섹션에 기재된 피드백 라인 중 `[룰문서]` 태그가 기재된 개정 사항을 룰 파일들(`GEMINI.md`, 매뉴얼 등)에 자가 개정 이력 형식으로 누적 저장합니다.
   ```bash
   python3 BuddhaStory/skills/buddha-translation-helper/scripts/run_pipeline.py --upgrade-workflow [2차로그파일.md]
   ```
...
```mermaid
graph TD
    A[2차 윤문 지시 수신] --> B[1단계: run_pipeline.py --init-draft2 실행]
    B --> C[term/[파일명]-term.md에서 2차 윤문용 권장 용어 기입]
    C --> D[2단계: draft-2 원고 윤문 및 문맥 리라이팅 실행]
    D --> E[3단계: run_pipeline.py --validate-draft2 실행]
    E --> F[log/draft-2/[파일명]-draft2-log.md 생성 및 draft-3 복사 완료]
    F --> G[4단계: run_pipeline.py --upgrade-workflow 실행]
    G --> H[GEMINI.md 및 매뉴얼 규칙 자가 패치 및 최신화 완료]
    H --> I[인간 감수자 draft-3 최종 검토 및 출판 확정]
```
...
* **경로**: `BuddhaStory/log/draft-1/` (1차) 및 `BuddhaStory/log/draft-2/` (2차)
* **작성 언어**: 한국어
* **파일명 규칙**:
  - **1차 로그**: `BuddhaStory/log/draft-1/[작업문서명]-log.md`  
    예: `gcb-kr-004-salutation-and-intention-log.md`
  - **2차 로그**: `BuddhaStory/log/draft-2/[작업문서명]-draft2-log.md`  
