...
## 7.1 OL-Source Memory / RAG Layer

Source Collection Layer가 제대로 작동하려면 OL-Runner 앞단에 로컬 자료 기억 계층이 필요하다. 이 계층은 독립 자율 에이전트라기보다, 원천자료를 색인하고 검색하고 근거 패킷을 만들어 OL-Runner와 OL-DESK에 제공하는 `OL-Source Memory` 또는 `Source Packet Builder`에 가깝다.

이 계층을 RAG라고 부를 수는 있지만, 핵심은 “AI가 알아서 판단한다”가 아니라 “인간 편집자가 판단할 근거를 찾아온다”이다.
...
- 원문 위치 보존
- 키워드 검색과 의미 검색
- 특정 사건, 인물, 장소, 교리어 관련 구절 검색
- 새 목차 항목별 관련 원천자료 후보 제시
- chapter dossier에 들어갈 source packet 후보 생성
- 검색 결과마다 source_basis 제공
- 검색 결과의 불확실성과 검토 필요 표시
- OL-DESK에서 확인 가능한 한국어 source memory log 생성
```

RAG Layer가 담당하지 말아야 할 일:
...
따라서 RAG 결과는 항상 `확정 근거`가 아니라 `검토 대상 근거 후보`로 표시한다. 불교문헌에서는 비슷한 단어가 나온다는 사실과 같은 교리적 의미를 가진다는 사실이 같지 않다. 이 차이를 UI와 로그에서 명확히 구분해야 한다.

초기 도입 원칙:

```text
- 처음부터 자율 RAG 에이전트를 만들지 않는다.
- 먼저 로컬 Source Index와 검색 UI를 만든다.
- 그 다음 source packet 생성기를 붙인다.
- 이후 OL-Runner의 source-align, chapter-dossier-build와 연결한다.
- 모든 결과는 인간 확인을 거친 뒤 Contents-Asset에 확정 저장한다.
```
...
data/source-memory/
= 로컬 RAG 색인 메타데이터, chunk 정보, 검색 설정

data/retrieval-logs/
= OL-Source Memory가 어떤 검색어, 어떤 chunk, 어떤 근거 후보를 반환했는지 기록

data/source-packets/
= 새 목차 항목이나 의미 단위에 연결된 원천자료 발췌 묶음
...
OL-Runner와 OL-Source Memory의 관계:

```text
OL-Source Memory
= 자료를 기억하고 찾아오는 검색·근거 계층

OL-Runner
= 검색 결과를 바탕으로 구조화 산출물을 만드는 실행 계층

OL-DESK
= 사람이 검토하고 승인하고 원고로 쓰는 편집 계층
```

OL-Runner는 대규모 원천자료를 매번 모델 문맥창 안에 직접 넣어 처리하려고 해서는 안 된다. 필요한 자료는 OL-Source Memory에서 검색하고, 검색 결과의 출처와 불확실성을 함께 받아야 한다.

OL-Runner가 RAG를 사용할 때 지켜야 할 원칙:

```text
- retrieval result는 후보로 취급한다.
- source_basis 없이 생성된 해석은 낮은 grounding_level로 표시한다.
- 검색 결과가 서로 충돌하면 하나로 평준화하지 않고 conflict로 남긴다.
- 검색 실패도 로그로 남긴다.
- 인간 확인이 필요한 경우 human_check_required를 true로 둔다.
```
...
Desk Mode는 별도의 Reader Mode를 전제하지 않는다. OL-DESK에서 읽기와 쓰기는 분리된 모드가 아니라, 같은 원고 화면 위에서 자연스럽게 오가는 하나의 작업이어야 한다. 감수자와 편집자는 완성 원고를 읽다가 바로 문장을 고치고, 단락을 옮기고, 저장하고, 다시 읽어야 한다.

따라서 Desk Mode의 핵심은 읽기 전용 화면이 아니라 `Zen Editor`이다.
...
- 타이포그래피, 줄 간격, 여백은 장시간 쓰기와 재독에 맞춘다.
...
```text
- 노션처럼 화면에 보이는 원고를 직접 수정한다.
- 별도의 큰 Markdown textarea에 갇히지 않는다.
- 문단, 제목, 인용, 목록, 대본 블록, 장면 블록을 블록 단위로 편집한다.
- Enter, Backspace, 드래그, 붙여넣기, 선택 편집이 자연스럽게 동작한다.
- 저장은 명시 저장과 자동 저장을 함께 지원한다.
- 저장 상태는 조용히 표시하되 원고 집중을 방해하지 않는다.
- 원고 변경 이력과 버전은 내부에 남기되 상시 노출하지 않는다.
```
...
- 인용/주석/대본/장면 블록
- 선택 영역 기준 AI 보조 요청
- 선택 영역 기준 용어/각주/태그 연결
- 문장 또는 문단 단위 버전 비교
- 자동 저장, 수동 저장, 저장 충돌 방지
- undo/redo
- 초안, 확정, 보류, 재검토 표시
- source_basis와 creative_distance를 접힘 메타정보로 연결
```

Desk Mode에서 에디터가 지켜야 할 선:

```text
- 에디터가 원고보다 강하게 보이면 안 된다.
- 보조 패널은 원고 위에 상시 떠 있지 않는다.
- AI 제안은 원고를 덮어쓰지 않고 선택 가능한 후보로만 제시한다.
- 원문 근거와 창작 거리 정보는 필요할 때만 확인한다.
- 최종 문장 결정권은 항상 인간 편집자에게 있다.
```
...
Reader Mode는 별도 모드로 두지 않는다. 최종 원고를 읽는 경험은 Desk Mode의 `Zen Editor` 안에서 처리한다. 필요하다면 편집 UI를 거의 숨긴 `focus state`를 제공하되, 그 상태에서도 클릭하면 즉시 수정할 수 있어야 한다.

## 18.8 OL-ATLAS 계승 전략

OL-ATLAS는 현재 OL 프로젝트 안에서 단일 HTML 기반 로컬 우선 콘텐츠 제작 도구로 작동한다. 카드, 칸반보드, 문서뷰, 태그 검색, prefix 태그, 독서형 화면, WORKS zip import를 이미 갖고 있다. 이 기능들은 OL-DESK 의미중심 전환에 직접적인 영감을 준다.

다만 OL-ATLAS를 그대로 OL-DESK의 전체 데이터 백본으로 삼는 것은 적절하지 않다. ATLAS는 카드형 로컬 저작 도구로는 강하지만, OL-DESK가 요구하는 source alignment, source packet, chapter dossier, meaning_unit, grounding, creative_distance, OL-Runner log 수신 구조를 그대로 갖고 있지는 않다.

따라서 권장 방향은 “소스코드 전면 이식”이 아니라 “상호작용 모델 계승”이다.

OL-DESK가 계승할 ATLAS의 핵심:

```text
- 칸반보드 기반 분류와 진행 흐름
- 카드 중심 자료/의미 단위 관리
- 컬럼, 그룹, 태그의 3단 분류 감각
- prefix 태그 기반 빠른 임시 분류
- 문서뷰의 집중 편집 경험
- 단일 HTML과 로컬 우선 철학
- WORKS zip import의 묶음 수집 아이디어
- 사이드바 기반 문서/태그 탐색
```

OL-DESK에서 새로 설계해야 할 부분:

```text
- 명시적 작업 타입: source_profile, source_toc, event, theme, source_packet, chapter_dossier, meaning_unit, rendering
- 관계 모델: source -> excerpt -> source_packet -> chapter_dossier -> meaning_unit -> rendering
- source_basis, grounding_level, creative_distance의 구조화 필드
- OL-Source Memory와 OL-Runner 결과 수신 포맷
- 다중 원천자료 비교 화면
- 장기 보관 가능한 Contents-Asset 저장 구조
- Zen Editor와 Source/Meaning Board의 왕복 UX
```
...
ATLAS형 Workspace
= 로컬 우선 보드, 카드, 원고 편집 경험의 UI 원형

OL-DESK
= ATLAS형 상호작용을 계승하되 Contents-Asset, OL-Runner, OL-Source Memory와 결합한 전문 편집 데스크
...
초기 실험에서는 ATLAS의 카드 모델을 참고해 `card.meta.type`, `sourceRefs`, `relations`, `groundingLevel`, `renderingType` 같은 개념을 OL-DESK UI 프로토타입에 반영할 수 있다. 그러나 최종 저장은 ATLAS localStorage가 아니라 Contents-Asset의 명시적 JSON 구조를 우선한다.
...
- data/source-tocs/ 초안
- data/source-memory/ 초안
- data/retrieval-logs/ 초안
- data/source-packets/ 초안
- data/event-index/ 초안
- data/theme-index/ 초안
...
## Phase 3. OL-Source Memory MVP

```text
- 로컬 원천자료 ingest
- chunk 생성과 원문 위치 보존
- source profile 메타데이터 연결
- 키워드 검색과 의미 검색
- 검색 결과의 source_basis 표시
- 검색 실패와 불확실성 로그
- Source Board에서 검색 결과 검토
```

## Phase 4. OL-DESK 의미 단위 뷰
...
## Phase 5. OL-Runner 자료 정리 MVP

```text
- OL-Source Memory 검색 결과 사용
- source-profile
- toc-extract
- event-index
- theme-index
...
## Phase 9. Desk / Board Mode 구현

```text
- Desk Mode Zen Editor 구현
- 화면 위 직접 수정과 저장
- 블록 기반 원고 편집
- Markdown 호환 입출력
- 문단/문장 단위 버전 비교
...
- 창작 거리와 원문 근거 표시
- 산출물 유형별 원고 편집 UX
- 별도 Reader Mode 없이 Desk Mode focus state로 재독 지원
```

## Phase 10. ATLAS 계승 UI 실험

```text
- 카드 중심 Source Board 프로토타입
- 칸반 기반 chapter dossier 진행 관리
- prefix 태그 기반 임시 분류
- ATLAS형 사이드바 탐색 구조 검토
- WORKS zip import 아이디어를 source collection import로 확장
- ATLAS localStorage가 아니라 Contents-Asset 저장 구조로 연결
```

## Phase 11. OL-TOON / OL-STUDIO 연계
